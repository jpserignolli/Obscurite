...
public class obscuriteProduto extends JFrame {
	...
	public obscuriteProduto() {
		...
		btnRegistro2.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
            	String categoria = (String)comboBoxProdutos.getSelectedItem();
            	String modelo = (String) comboBoxProdutos_1.getSelectedItem();
                String preco = textField.getText();

                if (preco.isEmpty() || categoria == null || categoria.isEmpty() || modelo == null || modelo.isEmpty()) {
                    JOptionPane.showMessageDialog(null, "Preencha todos os campos!", "Erro", JOptionPane.PLAIN_MESSAGE);
                } else {
                    setVisible(false);
                    
                    obscuriteVendas nextPage = new obscuriteVendas();
